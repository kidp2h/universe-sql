...
import { useQueryBenchmark } from "@/hooks/use-query-benchmark";
import { QueryBenchmarkResults } from "@/components/query-benchmark-results";
import {
  Activity,
  AlertCircle,
  Database,
...
export function QueryBenchmarkPage() {
  ...
  const {
    ...
    handleReset,
    winnerInfo,
  } = useQueryBenchmark(true, selectedConnId);
    ...
    <div className="flex flex-col h-full w-full bg-background overflow-y-auto p-6 space-y-6 animate-in fade-in duration-300">
      <div className="mb-2">
        <h2 className="flex items-center gap-2 text-2xl font-bold text-foreground animate-in slide-in-from-top-3 duration-300">
          <Zap className="size-6 text-indigo-500 animate-pulse" />
          {t("benchmarkTitle")}
        </h2>
      ...
      {!selectedConnId ? (
        ...
        <div className="space-y-6 flex-1 min-h-0">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-indigo-400 flex items-center gap-1.5">
                  <span className="flex items-center justify-center size-5 rounded bg-indigo-500/10 border border-indigo-500/20 text-sm">
                    A
                  </span>
                  {t("queryLabel")} A
                </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {errorA && (
                    <span className="text-sm text-rose-500 flex items-center gap-1">
                      <AlertCircle className="size-3" /> {t("syntaxError")}
                    </span>
                  )}
                </div>
              </div>
              ...
              >
                <CodeMirror
                  ...
                  height="100%"
                  placeholder="e.g. SELECT * FROM users JOIN orders ON users.id = orders.user_id WHERE users.active = true;"
                  readOnly={isRunning}
                />
              </div>
              {errorA && (
                <p className="text-sm text-rose-500/90 font-mono bg-rose-500/5 p-2.5 rounded-lg border border-rose-500/10">
                  {errorA}
                </p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-sky-400 flex items-center gap-1.5">
                  <span className="flex items-center justify-center size-5 rounded bg-sky-500/10 border border-sky-500/20 text-sm">
                    B
                  </span>
                  {t("queryLabel")} B
                </span>
              ...
              >
                <CodeMirror
                  ...
                  className="h-full text-sm font-mono"
                  height="100%"
                  placeholder="e.g. SELECT * FROM users WHERE active = true AND id IN (SELECT user_id FROM orders);"
                  readOnly={isRunning}
                />
              </div>
              {errorB && (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-xl border bg-muted/20">
            <div className="flex items-center gap-3">
              ...
              <Input
                ...
                value={iterations}
                onChange={(e) =>
                  setIterations(parseInt(e.target.value, 10) || 10)
                }
                disabled={isRunning}
                className="w-20 bg-background border-border text-center rounded-lg no-spinner"
              />
          ...
          {/* Results Section */}
          {result && !isRunning && (
            <QueryBenchmarkResults result={result} winnerInfo={winnerInfo} />
          )}
        </div>
      )}
    </div>
