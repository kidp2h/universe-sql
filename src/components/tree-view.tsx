...
const selectedTreeVariants = cva("bg-primary/20");
...
const TreeView = React.forwardRef<HTMLDivElement, TreeProps>(
  (
    ...
    logger.log(
      `[TreeView] Rendered: selectedItemId = ${selectedItemIdProp !== undefined ? selectedItemIdProp : initialSelectedItemId}`,
    );
    ...
    const selectedItemId =
      selectedItemIdProp !== undefined
        ? selectedItemIdProp
        : selectedItemIdInternal;
...
const TreeNode = React.memo(
  ({
    ...
    logger.log(`[TreeNode] Rendered: name = ${item.name}, id = ${item.id}`);
  ...
  (prevProps, nextProps) => {
    ...
    const wasSelected = prevProps.selectedItemId === prevProps.item.id;
    const isSelected = nextProps.selectedItemId === nextProps.item.id;
...
const TreeLeaf = React.memo(
  ...
  >(
    (
      ...
      logger.log(`[TreeLeaf] Rendered: name = ${item.name}, id = ${item.id}`);
      const [isDragOver, setIsDragOver] = React.useState(false);
      const isSelected = selectedItemId === item.id;
