...
export function useCellSelection(
  ...
  const selectionStartRef = React.useRef<{ row: number; col: number } | null>(
    null,
  );
  const selectionEndRef = React.useRef<{ row: number; col: number } | null>(
    null,
  );
  ...
  const updateSelectionDOM = React.useCallback(
    (startRow: number, startCol: number, endRow: number, endCol: number) => {
      ...
      const allCells =
        tableContainerRef.current?.querySelectorAll(".data-cell") || [];
      let sum = 0,
        count = 0,
        hasNonNumeric = false;
      let singleCellText = "";
      ...
      if (footerSummaryContentRef.current && footerSummaryRef.current) {
        if (count === 0) {
          ...
          const displaySum = new Intl.NumberFormat("en-US", {
        ...
        const jsonBtn = tableContainerRef.current
          ?.closest(".absolute")
          ?.querySelector('[data-slot="view-as-json-button"]');
        if (jsonBtn) {
          if (
            count === 1 &&
  ...
  const startScrollLoop = React.useCallback(() => {
    ...
    const loop = () => {
      ...
      if (x > rect.right - threshold)
        dx = Math.min(maxScroll, (x - (rect.right - threshold)) / 2);
      else if (x < rect.left + threshold)
        dx = -Math.min(maxScroll, (rect.left + threshold - x) / 2);

      if (y > rect.bottom - threshold)
        dy = Math.min(maxScroll, (y - (rect.bottom - threshold)) / 2);
      else if (y < rect.top + threshold)
        dy = -Math.min(maxScroll, (rect.top + threshold - y) / 2);
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        if (cell && selectionStartRef.current && selectionEndRef.current) {
          ...
          if (!isSelectCol && r !== -1 && c !== -1) {
            ...
            updateSelectionDOM(
              selectionStartRef.current?.row,
              selectionStartRef.current?.col,
              r,
              c,
            );
          }
  ...
  const getSelectedMatrix = React.useCallback(() => {
    ...
    const rowMap = new Map<
      number,
      { col: number; text: string; type: string }[]
    >();
  ...
  const handleCopyInStatement = React.useCallback(() => {
    ...
    rowMap.forEach((cols) => {
      cols.forEach((c) => {
        if (c.text !== "null") {
          values.push(
            c.type === "number" ? c.text : `'${c.text.replace(/'/g, "''")}'`,
          );
        }
      });
    });
