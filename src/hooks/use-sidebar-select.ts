  ...
  const handleSelectChange = React.useCallback(
    async (item: TreeDataItem | undefined) => {
      ...
      if (item.id.includes(":indexes")) {
        const schemaPath = item.id.split(":schema:")[0];
        const connId = schemaPath.includes(":db:")
          ? schemaPath.split(":db:")[0]
          : schemaPath;
      ...
      if (item.id.includes(":query:")) {
        const queryPath = item.id.split(":query:")[0];
        const connId = queryPath.includes(":db:")
          ? queryPath.split(":db:")[0]
          : queryPath;
        ...
        if (window.electron?.readQuery) {
          try {
            ...
            const existingTab = tabState.queryTabs.find(
              (t) => t.filePath === filePath,
            );
            if (existingTab) {
              if (existingTab.connectionId !== connId) {
                tabState.updateQueryTab({
                  ...existingTab,
                  connectionId: connId,
                });
              }
            ...
            if (res.ok && res.content !== undefined) {
              ...
              toast.error("Failed to read query file", {
